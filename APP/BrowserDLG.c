...
//	{ MENU_CreateIndirect, "Menu", ID_MENU_0, 0, 1, 230, 16, 0, 0x00, 0 },
	...
	{ TREEVIEW_CreateIndirect, "Treeview", ID_TREEVIEW_0, 0, 21, 230, 240, 0, 0x00, 0 },
...
static void _AddMenuItem(MENU_Handle hMenu, MENU_Handle hSubmenu, const char* pText, U16 Id, U16 Flags) {
	MENU_ITEM_DATA Item;
	MENU_SetFont(hMenu,&GUI_FontHZ12);//teng
	Item.pText    = pText;
...
static WM_HWIN _CreateMenu(WM_HWIN hParent) {
	...
	_AddMenuItem(hMenuEdit,   0,           "命名",    ID_MENU_EDIT_RENAME,    0);
	...
	_AddMenuItem(hMenuSet,    0,           "线条",     ID_MENU_SET_LINE,   0);
	_AddMenuItem(hMenuSet,    0,           "方式",     ID_MENU_SET_SEL,   0);
...
treeitemx = hItem;
	...
	TREEVIEW_SetFont(hItem,&GUI_FontHZ12);
...
void fun_MENU_ON_ITEMSELECT(WM_MESSAGE *pMsg, MENU_MSG_DATA *pData)
{
	...
	switch(pData->ItemId)
	{
		case ID_MENU_FILE_OPEN:
			MESSAGEBOX_Create("Sorry open", "About",0);
		break;
		case ID_MENU_FILE_ATTR:
			MESSAGEBOX_Create("Sorry attr", "About",0);
		break;
		case ID_MENU_FILE_EXIT:
			GUI_EndDialog(pMsg->hWin, 0);
		//	MESSAGEBOX_Create("Sorry exit", "About",0);
		break;
		case ID_MENU_EDIT_RENAME:
			MESSAGEBOX_Create("Sorry rename", "About",0);
		break;
		case ID_MENU_EDIT_MOVE:
			MESSAGEBOX_Create("Sorry move", "About",0);
		break;
		case ID_MENU_EDIT_COPY:
			MESSAGEBOX_Create("Sorry copy", "About",0);
		break;
		case ID_MENU_EDIT_DELETE:
			MESSAGEBOX_Create("Sorry delete", "About",0);
		break;                                                                     
		...
		case ID_MENU_HELP_ABOUT:
			...
			WM_DefaultProc(pMsg);
        // USER END
		//	MESSAGEBOX_Create("by WZTENG", "About",0);   //可以修改窗体属性 framewin
		break;
		default:
			WM_DefaultProc(pMsg); 
	}
...
//	MENU_ITEM_DATA         ItemData;
	...
	switch (pMsg->MsgId) {
		case WM_INIT_DIALOG:
			...
			exf_getfree("0:",&sdcardtotal,&sdcardfree);            //获取SDCard卡总量与空间容量
			exf_getfree("1:",&exflashtotal,&exflashfree);          //获取ExFlash卡总量与空间容量
			hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_1);
			sprintf(pbuff,"容量:%dKB",exflashtotal);
			TEXT_SetText(hItem, pbuff);
			hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_2);
			sprintf(pbuff,"剩余:%dKB",exflashfree);
			TEXT_SetText(hItem, pbuff);
				...
				case ID_TREEVIEW_0: // Notifications sent by 'Treeview'
					switch(NCode) {
						...
						// USER START (Optionally insert code for reacting on notification message)
							...
							if(strcmp(pbufft,"SDCard")==0)                                                  //SDCard
							{                                                                               //显示信息
								hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_1);
								sprintf(pbuff,"容量:%dKB",sdcardtotal);
								TEXT_SetText(hItem, pbuff);
								hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_2);
								sprintf(pbuff,"剩余:%dKB",sdcardfree);
								TEXT_SetText(hItem, pbuff);
							}else if(strcmp(pbufft,"ExFlash")==0)                                           //ExFlash
							{                                                                               //显示信息
								hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_1);
								sprintf(pbuff,"容量:%dKB",exflashtotal);
								TEXT_SetText(hItem, pbuff);
